  ...
  const handleRejectOrder = async (orderId) => {
    try {
      ...
      setOrders(prevOrders =>
        prevOrders.map(order =>
          order.id === orderId
            ? { ...order, status: 'rejected', rejectedAt: new Date() }
            : order
        )
      );
      console.log(`Pedido ${orderId} rechazado.`);
    ...
    <div style={{ display: 'flex' }}>
      ...
      <div className="orders-container">
        ...
        {!loading && !error && orders.length > 0 && (
          <div className="orders-list">
            {orders.map(order => {
              const isPaid = order.status === 'paid' || order.status === 'accepted';
              ...
              return (
                <div key={order.id} className="order-item">
                  ...
                  <p className="payment-status">
                    Pago:{' '}
                    {isPaid ? (
                      <span role="img" aria-label="pagado" style={{ color: 'green', fontWeight: 'bold' }}>
                        ✅ Pagado
                      </span>
                    ) : (
                      <span role="img" aria-label="no pagado" style={{ color: 'red', fontWeight: 'bold' }}>
                        ❌ No pagado
                      </span>
                    )}
                  </p>
                  ...
                  {order.acceptedAt && (
                    <p>
                      ...
                      {order.acceptedAt instanceof Date
                        ? order.acceptedAt.toLocaleDateString()
                        : order.acceptedAt?.toDate().toLocaleDateString()}
                    </p>
                  )}
                  {order.rejectedAt && (
                    <p>
                      ...
                      {order.rejectedAt instanceof Date
                        ? order.rejectedAt.toLocaleDateString()
                  ...
                  {/* Botones para aceptar/rechazar solo si está pagado y en estado 'pending' */}
                  ...
                  {order.status === 'pending' && !isPaid && (
                    <p className="order-status-message" style={{ color: '#f57c00', fontWeight: 'bold' }}>
                      Este pedido está pendiente, pero aún no fue pagado.
                    </p>
                  )}

                  {order.status !== 'pending' && (
                    <p className="order-status-message">
                      Este pedido ya ha sido {order.status === 'accepted' ? 'aceptado' : 'rechazado'}.
                    </p>
                  )}
                </div>
              );
